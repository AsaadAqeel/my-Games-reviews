...
function renderCard(played, details) {
...
async function render() {
  ...
  if (error) {
    console.error("Failed to load played games:", error.message);
    renderEmpty("Failed to load played games.");
    return;
  }
  ...
  if (list.length === 0) {
    renderEmpty("No played games yet.");
    return;
  }
  ...
  for (const played of list) {
    gridEl.appendChild(renderCard(played, details));
  }
}
